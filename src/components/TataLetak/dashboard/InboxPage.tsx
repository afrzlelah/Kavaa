...
import {
  cariPercakapanAdaKlien,
  tandaiPesanSudahDibacaKlien,
} from "@/services/layananPesanKlien";
...
export default function ChatApp({
          ...
          (payload: {
            ...
            if (newMsg.conversation_id === activeChat) {
              ...
              if (newMsg.sender_id !== userId) {
                tandaiPesanSudahDibacaKlien(newMsg.conversation_id, userId!);
              }
            }
  ...
  const handleSearchUsers = async (query: string) => {
    ...
    setIsSearching(true);
    const { createClientClient } = await import("@/utilitas/supabase/client");
    const supabase = createClientClient();
  ...
  const startNewConversation = async (otherUser: { id: string; first_name: string; last_name: string; avatar_url?: string }) => {
    ...
    try {
      ...
      const existingId = await cariPercakapanAdaKlien(
        userId!,
        otherUser.id,
      );
  ...
  const handleSelectChat = async (id: string) => {
    ...
    // 2. Mark as read
    await tandaiPesanSudahDibacaKlien(id, userId!);
            ...
            <div>
              ...
              <div className="space-y-1">
                {conversations.map((c) => (
                  <ContactItem
                    key={c.id}
                    contact={c as unknown as Kontak}
